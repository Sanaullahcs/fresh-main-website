...
export default function BuyPage() {
  ...
  const loadProperties = async (page: number = currentPage) => {
    ...
    try {
      ...
      const amenities = []
      if (filters.pool) amenities.push("pool")
      if (filters.garden) amenities.push("garden")
      if (filters.garage) amenities.push("garage")
  ...
  return (
    <div className="min-h-screen bg-white">
                  ...
                  {availableLocations.length > 0 && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Locations ({filters.location.length} selected)
                      </label>
                      <div className="relative">
                        <button
                        ...
                        >
                          <span className="text-gray-500">
                            {filters.location.length > 0
                              ? `${filters.location.length} location${filters.location.length > 1 ? "s" : ""} selected`
                              : "Select Locations"}
                          </span>
                          <ChevronDown className="h-4 w-4 text-gray-500" />
                        </button>
                        {dropdownStates.location && (
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-10 max-h-60 overflow-y-auto">
                            ...
                            {availableLocations.map((location) => (
                              <label
                                key={location}
                                className="flex items-center px-3 py-2 hover:bg-green-50 cursor-pointer text-sm"
                              >
                                <input
                                  type="checkbox"
                                  checked={filters.location.includes(location)}
                                  onChange={() => selectLocationOption(location)}
                      ...
                      <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                        ...
                        {["Any Type", "Apartment", "Commercial", "House", "Plot", "Townhouse"].map((option) => (
                          <button
                            ...
                            className={`w-full px-3 py-2 text-left hover:bg-green-50 transition-colors text-sm ${
                              (filters.propertyType === "" && option === "Any Type") || filters.propertyType === option
                          ...
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                            ...
                            {["Any", "1", "2", "3", "4", "5+"].map((option) => (
                              <button
                                ...
                                className={`w-full px-3 py-2 text-left hover:bg-green-50 transition-colors text-sm ${
                                  (filters.bedrooms === "" && option === "Any") || filters.bedrooms === option
                                    ? "bg-green-500 text-white"
                                    : "text-gray-700"
                                }`}
                              >
                          ...
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                            ...
                            {["Any", "1", "2", "3", "4+"].map((option) => (
                              <button
                                ...
                                className={`w-full px-3 py-2 text-left hover:bg-green-50 transition-colors text-sm ${
                                  (filters.bathrooms === "" && option === "Any") || filters.bathrooms === option
                                    ? "bg-green-500 text-white"
                                    : "text-gray-700"
                                }`}
                              >
      ...
      {/* Properties Section */}
      <section className="py-12 bg-white" id="properties-section" ref={propertiesSectionRef}>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
